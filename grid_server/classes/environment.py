...
class GridWorld:
    ...
    def action(self, action, entity):
        ...
        inventory = ['slot1', 'slot2', 'slot3', 'slot4', 'slot5', 'slot6', 'slot7', 'slot8', 'slot9']
        equip = ['head', 'chest', 'legs', 'feet', 'hands', 'cape', 'neck', 'ring', 'right', 'left']
        menu = ['drop', 'use', 'equip', 'unequip']
        if action in move:
            self.move(action, entity)
            data = ''
        elif action in inventory:
            data = self.menu(entity, action)
        elif action == 'interact':
            data = self.interact(entity)
        elif action == 'attack':
            attack_data = self.attack(entity)
        elif action == 'menu_drop':
            entity.menu_drop = not entity.menu_drop
            data = f"Menu drop is now {entity.menu_drop}."
        else:
    ...
    def interact(self, entity):
        ...
        player_tile = self.grid.get_tile(entity.x, entity.y)
        if player_tile.items:
            entity.pick_up(player_tile.items[0])
            text = f"Picked up a {player_tile.items[0]['name']}."
            player_tile.remove('item', player_tile.items[0])
            return text
    ...
    def attack(self, entity):
        ...
        for k, v in target_damage.items():
            for target in v['target']:
                ...
                if target_entity != entity and target_entity is not None:
                    ...
                    if is_dead:
                        ...
                        target_entity.x = target_entity.home[0]
                        target_entity.y = target_entity.home[1]
                        self.grid.get_tile(target_entity.home[0], target_entity.home[1]).set('entity', target_entity)
                        deaths.append(f"{target_entity.name} has died.")
                    combats.append(f"Engaged in combat with {target_entity.name}.")
        return combats + deaths
    ...
    def menu(self, player, action):
        slot = int(action[-1]) - 1
        if player.inventory.slots[slot] == {}:
            return "No item in slot."
        if player.menu_drop:
            dropped = player.drop(slot)
            self.grid.get_tile(player.x, player.y).set('item', dropped)
            return f"{player.name} dropped {dropped['name']}."
        else:
            player.use(slot)
            return f"{player.name} used item."
